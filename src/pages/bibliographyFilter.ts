...
class BibliographyFilter {
  ...
  <div class="bib-citation">${formatAsciidoc(e.citation)}</div>
    ...
    private sortComplete(): void {
        ...
        this.listEl?.classList.remove('sorting');
        this.stopPacmanTimer();
        // Sanity check: DOM order should match sortItems
        if (this.listEl) {
            const domOrder = Array.from(this.listEl.querySelectorAll<HTMLElement>('.bib-entry:not(.bib-hidden)'));
            const mismatch = this.sortItems.findIndex((el, i) => el !== domOrder[i]);
            if (mismatch >= 0) console.warn(`Sort complete: DOM/sortItems mismatch at index ${mismatch}`);
        }
        this.buildAzNav();
        if (this.terminal) {
