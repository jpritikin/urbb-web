...
const WIND_MAGNITUDE_MAX_PXPS = 18;
...
const WIND_DIRECTION_CHANGE_MIN_SEC = 25;
const WIND_DIRECTION_CHANGE_MAX_SEC = 60;

// Words evoking the page's themes (EQ, Self-leadership, provocation, trailheads)
// without repeating text already visible in the prose.
const CLOUD_WORDS = [
    'composure', 'reactivity', 'discernment', 'impulse', 'restraint',
    'presence', 'hijack', 'poise', 'clarity', 'unburdening',
    'trailhead', 'threshold', 'witness', 'steadiness', 'provocation',
    'Self-leadership', 'equanimity', 'temperance', 'grounding', 'vigilance',
];
...
export function initPromiseClouds(): () => void {
    ...
    function spawnCloud(x: number, y: number): DriftingCloud {
        ...
        const entry: DriftingCloud = { cloud, speedFactor: randomBetween(0.7, 1.3) };
        drifting.push(entry);
        return entry;
    }
