...
export class SimulatorController {
    ...
    private handleImpactRecognition(cloudId: string, targetCloudId: string): ControllerActionResult {
        ...
        if (interPartTrust >= 0.5) {
            const trustDiff = protectorTrust - noticerTrust;
            const successProb = trustDiff <= 0
                ? 1
                : Math.min(1, 0.5 + (4 * (interPartTrust - 0.5) - 0.5 * trustDiff));
            if (this.rng.random('notice_part:impact_success') < successProb) {
