...
export class CloudManager {
    ...
    private stretchTargetMarkers: Map<string, SVGCircleElement> = new Map();
    ...
    private animate(): void {
        ...
        if (mode === 'foreground') {
            ...
            this.updateStretchTargetMarkers();
            if (!isTransitioning) {
                this.updateMarkerElements();
            }
        } else {
            this.clearStretchTargetMarkers();
        }
    ...
    private updateStretchTargetMarkers(): void {
        if (!this.counterZoomGroup) return;
        ...
        for (const cloudId of blendedParts) {
            ...
            let marker = this.stretchTargetMarkers.get(cloudId);
            if (!marker) {
                marker = document.createElementNS('http://www.w3.org/2000/svg', 'circle');
                marker.setAttribute('r', '6');
                marker.setAttribute('fill', 'none');
                marker.setAttribute('stroke', '#ff00ff');
                marker.setAttribute('stroke-width', '2');
                marker.setAttribute('stroke-dasharray', '3,3');
                this.counterZoomGroup.appendChild(marker);
                this.stretchTargetMarkers.set(cloudId, marker);
            }
        ...
        // Remove markers for clouds no longer stretching
        for (const [cloudId, marker] of this.stretchTargetMarkers) {
            if (!activeCloudIds.has(cloudId)) {
                marker.remove();
                this.stretchTargetMarkers.delete(cloudId);
            }
        }
    }

    private clearStretchTargetMarkers(): void {
        for (const marker of this.stretchTargetMarkers.values()) {
            marker.remove();
        }
        this.stretchTargetMarkers.clear();
    }
}
